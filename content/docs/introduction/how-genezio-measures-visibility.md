...
Genezio measures **AI Visibility** by analyzing how often a brand appears in answers generated by AI systems during simulated conversations.

At its core, the metric answers a simple question:
...
Because the model will almost always reference those brands in the response, Comparer Agent conversations are also **excluded from visibility calculations**.

---

## Why Only Prompter Agent and Recommender Agent Conversations Count

To produce a meaningful metric, Genezio only includes conversations where the AI system decides **on its own** which brands to mention.

Prompter Agent and Recommender Agent conversations meet this requirement because:

* the user asks about a category
* the AI system chooses which brands to recommend
* the appearance of a brand reflects the model's understanding of the market

By excluding Introspector Agent and Comparer Agent conversations, the visibility metric focuses only on situations where brands appear organically.
